package io.github.fallout015.two.entity.item;
...
public class BoatEntityTwo extends Entity {
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE_TWO = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.VARINT);
	private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(BoatEntityTwo.class, DataSerializers.VARINT);
	...
	private BoatEntityTwo.Status status;
	private BoatEntityTwo.Status previousStatus;
	...
	public BoatEntityTwo(EntityType<? extends BoatEntityTwo> type, World world) {
		super(type, world);
	...
	protected void registerData() {
		...
		this.dataManager.register(BOAT_TYPE_TWO, BoatEntityTwo.Type.CRIMSON.ordinal());
	...
	public boolean isInvulnerableTo(DamageSource source) {
		return (this.getBoatType() == Type.CRIMSON || this.getBoatType() == Type.WARPED) && source == DamageSource.LAVA;
	}
	...
	public void applyEntityCollision(Entity entityIn) {
		if (entityIn instanceof BoatEntityTwo) {
	...
	public Item getItemBoat() {
		switch(this.getBoatType()) {
			case CRIMSON:
			default:
				return ItemsTwo.CRIMSON_BOAT;
			case WARPED:
				return ItemsTwo.WARPED_BOAT;
		}
	}
	...
	public void tick() {
		...
		if (this.status != BoatEntityTwo.Status.UNDER_WATER && this.status != BoatEntityTwo.Status.UNDER_FLOWING_WATER) {
	...
	private BoatEntityTwo.Status getBoatStatus() {
		BoatEntityTwo.Status boatentity$status = this.getUnderwaterStatus();
		if (boatentity$status != null) {
			this.waterLevel = this.getBoundingBox().maxY;
			return boatentity$status;
		} else if (this.checkInWater()) {
			return BoatEntityTwo.Status.IN_WATER;
		} else {
			float f = this.getBoatGlide();
			if (f > 0.0F) {
				this.boatGlide = f;
	            return BoatEntityTwo.Status.ON_LAND;
			} else {
	            return BoatEntityTwo.Status.IN_AIR;
			}
		}
	}
	...
	@Nullable
	private BoatEntityTwo.Status getUnderwaterStatus() {
		AxisAlignedBB axisalignedbb = this.getBoundingBox();
				...
				for(int i2 = i1; i2 < j1; ++i2) {
					...
					if (fluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$mutable.getY() + fluidstate.getActualHeight(this.world, blockpos$mutable))) {
						if (!fluidstate.isSource()) {
							return BoatEntityTwo.Status.UNDER_FLOWING_WATER;
						}
		...
		return flag ? BoatEntityTwo.Status.UNDER_WATER : null;
	}
	...
	private void updateMotion() {
		...
		double d2 = 0.0D;
		this.momentum = 0.05F;
		if (this.previousStatus == BoatEntityTwo.Status.IN_AIR && this.status != BoatEntityTwo.Status.IN_AIR && this.status != BoatEntityTwo.Status.ON_LAND) {
			...
			this.lastYd = 0.0D;
			this.status = BoatEntityTwo.Status.IN_WATER;
		} else {
			if (this.status == BoatEntityTwo.Status.IN_WATER) {
				d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
	            this.momentum = 0.9F;
			} else if (this.status == BoatEntityTwo.Status.UNDER_FLOWING_WATER) {
	            d1 = -7.0E-4D;
	            this.momentum = 0.9F;
			} else if (this.status == BoatEntityTwo.Status.UNDER_WATER) {
	            d2 = (double)0.01F;
	            this.momentum = 0.45F;
			} else if (this.status == BoatEntityTwo.Status.IN_AIR) {
	            this.momentum = 0.9F;
			} else if (this.status == BoatEntityTwo.Status.ON_LAND) {
	...
	protected void readAdditional(CompoundNBT compound) {
		if (compound.contains("Type", 8)) {
			this.setBoatType(BoatEntityTwo.Type.getTypeFromString(compound.getString("Type")));
		}
	}
		...
		if (!this.isPassenger()) {
			if (onGroundIn) {
				if (this.fallDistance > 3.0F) {
					if (this.status != BoatEntityTwo.Status.ON_LAND) {
	...
	public void setBoatType(BoatEntityTwo.Type boatType) {
		this.dataManager.set(BOAT_TYPE_TWO, boatType.ordinal());
	}

	public BoatEntityTwo.Type getBoatType() {
		return BoatEntityTwo.Type.byId(this.dataManager.get(BOAT_TYPE_TWO));
	}
	...
	public boolean canSwim() {
		return this.status == BoatEntityTwo.Status.UNDER_WATER || this.status == BoatEntityTwo.Status.UNDER_FLOWING_WATER;
	}
	...
	public static enum Type {
		...
		public static BoatEntityTwo.Type byId(int id) {
			BoatEntityTwo.Type[] aboatentity$type = values();
		...
		public static BoatEntityTwo.Type getTypeFromString(String nameIn) {
			BoatEntityTwo.Type[] aboatentity$type = values();
